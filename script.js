...
if (scene == 0) {
					...
					text.innerHTML = 'sini, aku mau kasih ela kekuatan buat muterin langit';
					play();
				} else if (scene == 13) {
					...
					const actionInterval = setInterval(function () {
						if (count == 3) {
							...
							text.innerHTML = 'gimana? cantik gak sayangku?';
						}
						count++;
					}, 500);
					...
					const actionInterval = setInterval(function () {
						...
						if (count > 0) text.innerHTML = count;
							...
							text.style.fontSize = '30px';
							text.innerHTML = 'Selamat Tahun Baru Dan Ulang Tahun yang ke 21';
							clearInterval(actionInterval);
							scene = 22;
						}
						count--;
					...
					text.innerHTML = 'WKWK ela wish apa? Atau gak sempet?';
					...
					text.innerHTML = 'tuhhh aku spam-in bintang jatuh buat ela';
					btn.style.display = 'none';
					shoots.classList = 'shootingStar';
					shoots.innerHTML = '<span></span><span></span><span></span><span></span><span></span>';
					play();
				} else if (scene == 36) {
					text.innerHTML = 'biar elaku bisa make wishes sebanyak mungkinnn';
					play();
				} else if (scene == 37) {
